...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ch_") ; then
       CH_BUILD=$(echo -n $1 | sed -e 's/^ch_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $CH_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CH_BUILD=
    fi
    export CH_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    CH_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/ch/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CH model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch ch_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CH_VERSION)
        ZIPFILE=ch-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
